    ...
    match &st.act {
        Action::Test => {
            ...
            st.wd
                ...
                .await?;
            st.wd.goto(st.url.join("/index.php/settings/admin?sectionid=general")?.as_str()).await?;
            st.wd
                .screenshot(&st.ssp.join("screenshot-settings.png"))
                .await?;
            st.wd.goto(st.url.join("/index.php/settings/apps")?.as_str()).await?;

            (st.wd.query(By::Css("div.section:nth-child(1) > div:nth-child(5) > button:nth-child(1)")).first().await?)
                .wait_until()
                .displayed()
                .await?;
            st.wd
