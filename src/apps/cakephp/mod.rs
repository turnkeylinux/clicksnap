...
pub struct CakePHPRunner {}

#[async_trait]
impl Runner for CakePHPRunner {
    async fn exec(&self, st : &State) -> WebDriverResult<()> {
        match &st.act {
            Action::Test => {
                ...
                st.wait(By::XPath("//h3[contains(text(), 'Training')]")).await?.scroll_into_view().await?;
                st.wd.screenshot(&st.ssp.join("screenshot-docs.png")).await?;
                // adminer
                ...
                (st.wd.find(By::Name("auth[password]")).await?).send_keys(&st.pse.root_pass).await?;
