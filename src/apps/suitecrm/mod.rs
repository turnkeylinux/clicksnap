    ...
    match &st.act {
        Action::Test => {
            ...
            st.wd
                ...
                .await?;
            st.wait(By::XPath("//input[@name='username']")).await?.send_keys("admin").await?;
            st.wait(By::XPath("//input[@name='password']")).await?.send_keys(&st.pse.app_pass).await?;
            ...
            st.wait(By::XPath("//scrm-label[@module='administration']")).await?;

            st.wd
                .screenshot(&st.ssp.join("screenshot-admin.png"))
                .await?;

            st.wd.goto(st.url.join("#/accounts/edit?return_module=Accounts&return_action=DetailView")?).await?;
