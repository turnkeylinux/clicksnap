...
pub struct OwnCloudRunner {}

#[async_trait]
impl Runner for OwnCloudRunner {
    async fn exec(&self, st : &State) -> WebDriverResult<()> {
        match &st.act {
            Action::Test => {
                st.wd.goto(st.url.as_str()).await?;
                (st.wd.find(By::Id("user")).await?).send_keys("admin").await?;
                ...
                st.wd.screenshot(&st.ssp.join("screenshot-files.png")).await?;
                ...
                st.wd.goto(u.as_str()).await?;
                (st.wd.query(By::ClassName("app-preview")).first().await?).wait_until().displayed().await?;
                st.wd.screenshot(&st.ssp.join("screenshot-market.png")).await?;
                Ok(())
            },
            Action::Install => {
