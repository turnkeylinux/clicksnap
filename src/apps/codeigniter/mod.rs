...
pub struct CodeIgniterRunner {}

#[async_trait]
impl Runner for CodeIgniterRunner {
    async fn exec(&self, st : &State) -> WebDriverResult<()> {
        match &st.act {
            Action::Test => {
                ...
                // about/docs section
                st.wait(By::XPath("//footer")).await?.scroll_into_view().await?;
                st.wd.screenshot(&st.ssp.join("screenshot-about.png")).await?;
